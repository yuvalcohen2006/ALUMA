...
import { Search, Shield, User as UserIcon, Heart, ClipboardList } from "lucide-react";
  ...
  return (
    <AdminLayout>
      <header className="flex items-center justify-between flex-wrap gap-4 mb-8">
        <div>
          <p className="text-[10px] tracking-[0.3em] text-primary uppercase mb-1">Customers</p>
          <h1 className="font-display text-3xl">חברי מועדון</h1>
        </div>
                ...
                <tr key={r.id} className="border-t border-border hover:bg-muted/30">
                  ...
                  <td className="px-4 py-3">
                    {r.role === "admin" ? (
                      ...
                      <Badge variant="secondary" className="gap-1"><UserIcon className="w-3 h-3" />לקוח</Badge>
                    )}
                  </td>
      ...
      <Dialog open={!!selected} onOpenChange={(o) => !o && setSelected(null)}>
        <DialogContent dir="rtl" className="max-w-2xl max-h-[80vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle className="font-display text-2xl">{selected?.full_name || "לקוח"}</DialogTitle>
